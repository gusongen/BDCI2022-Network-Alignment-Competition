...
def customized_loss_repeat_loss(a1_embedding, a2_embedding, a1_align, a2_align, neg1_left, neg1_right, neg2_left, neg2_right, neg_samples_size, pos_margin=0.01, neg_margin=3, neg_param=0.2, only_pos=False):
    a1_embedding = F.normalize(a1_embedding, p=2, dim=1)
    a2_embedding = F.normalize(a2_embedding, p=2, dim=1)
    # process the ground truth
    a1_align = np.array(a1_align)
    ...
    # repeat loss, only allow G2 anchor show one time
    similarity_matrix = torch.mm(a1_embedding, a1_embedding.t())
    similarity_matrix = softmax(similarity_matrix, dim=1)
    repeat_cnt = torch.sum(similarity_matrix, dim=0)  # ideal [1,1,1,...,1,1]
    # repeat_loss=torch.sum(torch.abs(repeat_cnt -torch.ones_like(repeat_cnt)))
    repeat_loss = len(repeat_cnt)*mse_loss(repeat_cnt, torch.ones_like(repeat_cnt))
    # print('@repeat_loss', repeat_loss)
    return loss1 + loss2 + 1000 * repeat_loss
