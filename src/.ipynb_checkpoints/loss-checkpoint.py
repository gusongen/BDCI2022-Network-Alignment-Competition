...
def custom_loss_trusted_err(err_left, err_right, trusted_left, trusted_right,
                            a1_embedding, a2_embedding, a1_align, a2_align, neg1_left, neg1_right, 
                            neg2_left, neg2_right, neg_samples_size, pos_margin=0.01, 
                            neg_margin=3, neg_param=0.2, only_pos=False):
    ...
    err_left = torch.tensor(err_left)
    err_right = torch.tensor(err_right)
    trusted_left = torch.tensor(trusted_left)
    trusted_right = torch.tensor(trusted_right)
    
        # *trusted loss computation
    left_x = a1_embedding[trusted_left.long()]
    right_x = a2_embedding[trusted_right.long()]
    trusted_loss = torch.abs(left_x - right_x)
    trusted_loss = torch.sum(trusted_loss, dim=1)
        # *error loss computation
    left_x = a1_embedding[err_left.long()]
    right_x = a2_embedding[err_right.long()]
    err_loss = torch.abs(left_x - right_x)
    err_loss = torch.sum(err_loss, dim=1)
    ...
    right_x = a2_embedding[a2_align.long()]
    pos_loss = torch.abs(left_x - right_x)
    # print("pos_loss.shape:",pos_loss.shape) #torch.Size([3630, 128])
    pos_loss = torch.sum(pos_loss, dim=1)
    # print("pos_loss.shape:",pos_loss.shape) #torch.Size([3630])
    ...
    loss3 = 20*torch.sum(F.relu(2*trusted_loss-2*err_loss))
